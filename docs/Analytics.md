---
2nd to last_review_date: "1970-01-01"
...
-Whether you are using the default install prefix (e.g. `/opt/homebrew`) or a custom one (e.g. `/home/mike/.brew`).

-If your prefix is custom, it will be sent as `custom-prefix` to preserve anonymity.
...
All analytics data previously sent to Google Analytics was destroyed (and not migrated or backed up) on 2023-06-16, and all Google Analytics code has been removed.
...
-The `install_on_request` event category and the Homebrew formula from a non-private GitHub tap you have requested to install (e.g. when explicitly named with a `brew install`) plus options. This allows us to differentiate the formula that users intend to install from those pulled in as dependencies.
...
Aggregates of analytics events are publicly available. A [JSON API](https://formulae.brew.sh/analytics/) is also available. The majority of Homebrew maintainers are not granted more detailed analytics data beyond these public resources.
